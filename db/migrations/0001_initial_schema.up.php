 ...
 * Headhunter schema.
 *
 * The database has exactly one client: the API. There is one login role, it
 * has no password, and it is reachable only from an internal Docker network.
 * So there is no row level security and no per-user database role here:
 * users, sessions and permissions are ordinary application data.
 */
return function (PDO $pdo): void {
    $pdo->exec(<<<'SQL'
        ...
        -- Bearer tokens. Only the SHA-256 of a token is ever stored, so a database
        -- dump does not hand anyone a working credential.
